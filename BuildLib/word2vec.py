...
"--train_data=./_Data/text8.txt" "--eval_data=./_Data/questions-words.txt" "--save_path=./Summery"

flags.DEFINE_string("save_path", "./Summery",         "Directory to write the model and training summaries.")
...
flags.DEFINE_string("eval_data", "./_Data/questions-words.txt", "File consisting of analogies of four tokens."
    "embedding 2 - embedding 1 + embedding 3 should be close to embedding 4. See README.md for how to get 'questions-words.txt'.")
...
flags.DEFINE_float("subsample",     1e-3, "Subsample threshold for word occurrence."
                     "Words that appear with higher frequency will be randomly down-sampled. Set to 0 to disable.")

flags.DEFINE_boolean("interactive", False, "If true, enters an IPython interactive session to play with the trained model. E.g."
                                           ", try model.analogy(b'france', b'paris', b'russia') and model.nearby([b'proton', b'elephant', b'maxwell'])")

flags.DEFINE_integer("statistics_interval",  3,"Print statistics every n seconds.")
flags.DEFINE_integer("summary_interval",     3, "Save training summary to file every n seconds (rounded up to statistics interval).")
